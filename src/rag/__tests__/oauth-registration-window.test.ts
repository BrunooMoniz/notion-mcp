...
test("open sets expiry to now+ttl; open is exclusive of the expiry instant", () => {
  closeRegistrationWindow();
  const expiry = openRegistrationWindow(5 * 60_000, 1_000_000);
...
test("extend-only: a shorter, later call never shortens a longer window", () => {
  closeRegistrationWindow();
  openRegistrationWindow(60 * 60_000, 1_000_000); // operator: +60min
  const after = openRegistrationWindow(5 * 60_000, 1_100_000); // friend: +5min, earlier expiry
...
test("extend: a later call CAN push the expiry further out", () => {
  closeRegistrationWindow();
  openRegistrationWindow(5 * 60_000, 1_000_000);
  const after = openRegistrationWindow(5 * 60_000, 1_200_000);
  assert.equal(after, 1_200_000 + 5 * 60_000);
});

test("close forces the window shut", () => {
  openRegistrationWindow(60 * 60_000, 1_000_000);
  closeRegistrationWindow();
